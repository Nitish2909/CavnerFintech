...
import {
  Phone, Search, Menu, X, ChevronDown, Home, HandCoins, TrendingUp,
  Building, Sun, Briefcase, GraduationCap, IndianRupee, BriefcaseBusiness,
  CreditCard, User, LogOut, LayoutDashboard, FileText,
} from "lucide-react";
...
const navItems = [
  ...
  {
    label: "Investments Plan", href: "/investment",
    subMenu: [
  ...
  {
    label: "Corporate Finance", href: "/corporate",
    subMenu: [
  ...
  },
  {
    ...
    subMenu: [
      ...
      { label: "Education Loan", href: "/loans/education-loan", icon: GraduationCap },
      { label: "Business Loan", href: "/loans/business-loan", icon: BriefcaseBusiness },
  ...
  },
  {
    ...
    subMenu: [
      ...
      { label: "PM-Vidyalaxmi Portal", href: "/government-services/pm-vidyalaxmiportal", icon: Building },
      { label: "Stand-Up India Scheme", href: "/government-services/standup-india-scheme", icon: Building },
    ],
  },
  {
    label: "Others Services", href: "/others-services",
    subMenu: [
      ...
      <div className="bg-brand-800 text-white text-sm hidden md:block">
        <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <Link to="/partner-login" className="hover:text-white transition-colors">Partner Login</Link>
            <Link to="/partner" className="hover:text-white transition-colors">Become a Partner</Link>
            <a href="tel:918816942362" className="flex items-center gap-1 hover:text-white">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            ...
            <Link to="/" className="flex items-center gap-2.5 shrink-0">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-brand-600 to-brand-800 flex items-center justify-center text-white font-bold text-sm">
                CWF
              </div>
            ...
            <div className="flex items-center gap-3">
              {user ? (
                <div className="hidden md:flex items-center gap-2">
                  <Link to="/user/dashboard" className="flex items-center gap-1.5 px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">
                    <LayoutDashboard size={16} /> Dashboard
                  </Link>
                  <button onClick={handleLogout} className="flex items-center gap-1.5 px-3 py-2 text-sm text-rose-600 hover:bg-rose-50 rounded-lg">
        ...
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 max-w-[85%] bg-white overflow-y-auto">
            ...
            <div className="p-4 space-y-1">
                ...
                <div key={item.label}>
                  {item.subMenu ? (
                    <>
                      ...
                      >
                        {item.label}
                        <ChevronDown size={16} className={activeSubMenu === item.label ? "rotate-180" : ""} />
                      </button>
              ...
              {user ? (
                <>
                  <Link to="/user/dashboard" onClick={() => setOpen(false)} className="flex items-center gap-2 px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50 rounded-lg">
                    <LayoutDashboard size={16} /> Dashboard
                  </Link>
                  <button onClick={handleLogout} className="flex items-center gap-2 w-full px-3 py-2.5 text-sm text-rose-600 hover:bg-rose-50 rounded-lg">
                    <LogOut size={16} /> Logout
                  </button>
                </>
              ) : (
                <div className="flex gap-2 pt-2">
                  <Link to="/login" onClick={() => setOpen(false)} className="flex-1 text-center px-3 py-2 text-sm font-medium text-brand-700 border border-brand-300 rounded-lg">Login</Link>
                  <Link to="/register" onClick={() => setOpen(false)} className="flex-1 text-center px-3 py-2 text-sm font-medium text-white bg-brand-700 rounded-lg">Register</Link>
                </div>
              )}
            </div>
          </div>
