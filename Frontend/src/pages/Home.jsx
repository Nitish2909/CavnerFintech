...
import { ArrowRight, ShieldCheck, Clock, IndianRupee, TrendingUp, Users, Award, CheckCircle2 } from "lucide-react";
...
const Home = () => {
  const loanTypes = [
    { name: "Personal Loan", desc: "Quick funds for any need", rate: "10.5% p.a.", href: "/loans/personal-loan", icon: IndianRupee },
    { name: "Home Loan", desc: "Own your dream home", rate: "8.4% p.a.", href: "/loans/home-loan", icon: TrendingUp },
    { name: "Education Loan", desc: "Study in India or abroad", rate: "9.0% p.a.", href: "/loans/education-loan", icon: Award },
    { name: "Business Loan", desc: "Grow your business", rate: "11.0% p.a.", href: "/loans/business-loan", icon: Users },
  ];

  const features = [
    { icon: ShieldCheck, title: "Bank-Grade Security", desc: "Your data is protected with 256-bit encryption and RBI-compliant processes." },
    { icon: Clock, title: "Quick Approval", desc: "Get instant approval on eligible loans with minimal documentation." },
    ...
    <>
      ...
      <div className="text-center max-w-[40rem] mx-auto mb-10 md:mb-14">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {loanTypes.map((loan) => {
            ...
            return (
              <Link key={loan.name} to={loan.href} className="group bg-white rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-md hover:border-brand-300 transition-all">
                ...
                <p className="text-sm font-semibold text-brand-700">Starting {loan.rate}</p>
      ...
      <section className="relative bg-gradient-to-br from-brand-800 via-brand-700 to-brand-900 text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 py-16 lg:py-24 relative">
          <div className="grid lg:grid-cols-2 gap-10 items-center">
            <div>
              <span className="inline-block px-3 py-1 bg-brand-500/30 text-brand-100 text-sm rounded-full mb-4">India's Trusted Fintech Platform</span>
              <h1 className="text-4xl lg:text-5xl font-bold leading-tight mb-4">
                Smart Loans, Cards & Investments for Every Indian
              </h1>
              <p className="text-brand-100 text-lg mb-8 max-w-lg">
                Compare and apply for personal loans, home loans, credit cards, and investment plans from 40+ banks. Quick approval, best rates, zero hassle.
              </p>
              <div className="flex flex-wrap gap-3">
                <Link to="/loans" className="flex items-center gap-2 px-6 py-3 bg-white text-brand-800 font-semibold rounded-lg hover:bg-brand-50 transition-colors">
                  Apply Now <ArrowRight size={18} />
                </Link>
                <Link to="/emi" className="px-6 py-3 border border-brand-400 text-white font-semibold rounded-lg hover:bg-brand-600/30 transition-colors">
                  EMI Calculator
                </Link>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.title} className="bg-white rounded-2xl p-6 text-center">
      ...
      <section className="max-w-7xl mx-auto px-4 py-16">
        <div className="bg-gradient-to-r from-brand-700 to-brand-900 rounded-3xl p-10 lg:p-16 text-center text-white">
          ...
          <p className="text-brand-100 mb-8 max-w-xl mx-auto">Register today and get pre-approved offers from top banks within minutes.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link to="/register" className="px-6 py-3 bg-white text-brand-800 font-semibold rounded-lg hover:bg-brand-50">Register Now</Link>
            <Link to="/cibil" className="px-6 py-3 border border-brand-300 text-white font-semibold rounded-lg hover:bg-brand-600/30">Check CIBIL Score</Link>
          </div>
        </div>
      </section>
