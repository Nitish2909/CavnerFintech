...
import { 
  Plus, Search, X, Edit2, Trash2, AlertCircle, Users, Mail, Phone, MapPin, Calendar, MessageSquare
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-600 antialiased py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] overflow-hidden">
          ...
          {/* Mobile-First Layout: Card view on smaller viewports */}
          <div className="block lg:hidden divide-y divide-slate-100 bg-white">
            {paginatedData.length > 0 ? (
              paginatedData.map((member, index) => (
                <div key={member.id} className="p-5 space-y-4 hover:bg-slate-50/40 transition-colors">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-1">
                      <div>
                        <span className="text-xs font-medium text-slate-400 block mb-0.5">Mobile No.</span>
                      ...
                      <div>
                        <span className="text-xs font-medium text-slate-400 block mb-0.5">Team Size</span>
                    ...
                    <div className="pt-1">
                      <span className="text-xs font-medium text-slate-400 block">E-Mail ID</span>
                    ...
                    {member.address && (
                      <div className="pt-1">
                        <span className="text-xs font-medium text-slate-400 block">Address</span>
                        <span className="text-slate-600 truncate block font-medium flex items-center gap-1.5">
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-100">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ring-1 ring-inset ${
                      member.status === "Active" ? "bg-emerald-50 text-emerald-700 ring-emerald-600/10" : "bg-slate-100 text-slate-600 ring-slate-500/10"
                    }`}>
                      {member.status}
                    </span>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ring-1 ring-inset ${
                    ...
                    <span className="inline-flex items-center text-xs font-medium text-slate-600 ml-auto">
                      <span className={`h-1.5 w-1.5 rounded-full mr-1.5 ${member.whatsapp === "Enabled" ? "bg-emerald-500" : "bg-slate-400"}`} />
                      WA: {member.whatsapp}
                    </span>
                  </div>
                </div>
                ...
                {paginatedData.length > 0 ? (
                  paginatedData.map((member, index) => (
                    <tr key={member.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 font-semibold text-slate-900 truncate" title={member.name}>
                        {member.name}
                      </td>
                      <td className="px-6 py-4 space-y-1">
                        ...
                        <div className="text-slate-400 text-xs truncate flex items-center gap-1.5" title={member.email}>
                          <Mail className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                          {member.email}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-slate-500 truncate" title={member.address}>
                        <div className="flex items-center gap-1.5">
                      ...
                      <td className="px-4 py-4 text-center whitespace-nowrap">
                        <span className={`inline-flex items-center justify-center text-xs font-semibold ${
                          member.whatsapp === "Enabled" ? "text-emerald-700" : "text-slate-500"
                        }`}>
                          <span className={`h-1.5 w-1.5 rounded-full mr-1.5 ${member.whatsapp === "Enabled" ? "bg-emerald-500" : "bg-slate-400"}`}></span>
                          {member.whatsapp}
                        </span>
                      </td>
                  ...
                  <tr>
                    <td colSpan="11" className="px-6 py-16 text-center text-slate-400 font-medium">
          ...
          <div className="p-4 bg-slate-50/40 border-t border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm font-medium text-slate-500 select-none">
            <div className="text-center sm:text-left">
              Showing <span className="font-semibold text-slate-800">{filteredTeam.length === 0 ? 0 : (currentPage - 1) * entriesPerPage + 1}</span> to{" "}
              <span className="font-semibold text-slate-800">{Math.min(currentPage * entriesPerPage, filteredTeam.length)}</span> of{" "}
              <span className="font-semibold text-slate-800">{filteredTeam.length}</span> entries
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full border border-slate-200/80 overflow-hidden max-h-[90vh] flex flex-col">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Modify team identity configuration attributes.</p>
              </div>
              <button 
            ...
            <form onSubmit={handleUpdateMember} className="p-6 space-y-4 overflow-y-auto">
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs transition-all">
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center border border-slate-200/80">
            ...
            <h3 className="text-base font-bold text-slate-900 mb-1">Delete Profile</h3>
            <p className="text-sm text-slate-500 mb-6 px-2">
              Are you sure you want to remove this profile? This operational data removal sequence cannot be reversed.
            </p>
